...
class CreatePropertyViewModel @Inject constructor(
    ...
    companion object{
        ...
        private const val IMAGE_MAX_WIDTH = 600
        private const val IMAGE_MAX_HEIGHT = 600
    }
    ...
    private val _uiState = MutableStateFlow<CreatePropertyUIState>(
        CreatePropertyUIState.FormState(
            ...
            dateEntry = InstantFieldState(null, true),
            dateSale = InstantFieldState(null, true),
            apartmentNumber = FieldState("", true),
            location = LocationFormState(
                street = FieldState("", true),
                streetNumber = FieldState("", true),
    ...
    private fun loadData(){
        viewModelScope.launch {
            ...
            if (estateType.isSuccess && commodities.isSuccess && agents.isSuccess){
                ...
                _uiState.value = CreatePropertyUIState.Error(sError = "Unable to fetch date", formState = getFormState() )
            }
        }
    }
    ...
    fun updateFieldValue(fieldName: String, instant: Instant?){
        viewModelScope.launch {
            ...
            val updatedState = when(fieldName){
                "dateEntry" -> currentState.copy(dateEntry = formValidator.validateInstant(instant, ENTRY_DATE_MIN, ENTRY_DATE_MAX, ENTRY_DATE_IS_REQUIRED))
    ...
    fun updateFieldValue(fieldName: String, newValue: String){
        viewModelScope.launch {
            ...
            val updatedState = when(fieldName){
                ...
                "location.streetNumber" -> currentState.copy(location = currentState.location.copy(streetNumber = formValidator.validateInteger(newValue,LOCATION_NUMBER_MIN, LOCATION_NUMBER_MAX, LOCATION_NUMBER_IS_REQUIRED)))
    ...
    private fun validateAll(){
        ...
        currentState = currentState.copy(
            ...
            dateEntry = formValidator.validateInstant(currentState.dateEntry.value, ENTRY_DATE_MIN, ENTRY_DATE_MAX, ENTRY_DATE_IS_REQUIRED),
            dateSale = formValidator.validateInstant(currentState.dateSale.value, SALE_DATE_MIN, SALE_DATE_MAX, SALE_DATE_IS_REQUIRED ),
            ...
            location = currentState.location.copy(
                ...
                streetNumber = formValidator.validateInteger(currentState.location.streetNumber.value,LOCATION_NUMBER_MIN, LOCATION_NUMBER_MAX, LOCATION_NUMBER_IS_REQUIRED),
    ...
    private fun isFormValid(updatedState: CreatePropertyUIState.FormState): Boolean {
        ...
        Log.d(TAG, "entryDate: ${updatedState.dateEntry.isValid}")
        Log.d(TAG, "saleDate: ${updatedState.dateSale.isValid}")
        ...
        Log.d(TAG, "location.streetNumber: ${updatedState.location.streetNumber.isValid}")
        ...
        return updatedState.name.isValid &&
                ...
                updatedState.dateEntry.isValid &&
                updatedState.dateSale.isValid &&
                updatedState.apartmentNumber.isValid &&
                updatedState.location.street.isValid &&
                updatedState.location.streetNumber.isValid &&
    ...
    fun saveProperty() {
        viewModelScope.launch {
            ...
            val currentState = getFormState()
            var isSold = false
            if (currentState.dateSale.value != null){
                isSold = true
            }
            if(currentState.isFormValid){
                val property = Property(
                    ...
                    dateEntry = currentState.dateEntry.value,
                    ...
                    location = Location(
                        ...
                        streetNumber = formConverter.toInt(currentState.location.streetNumber.value),
                ...
                try {
                    if(editModeIDLoaded > 0){
                        ...
                        Log.d("CPVM", "Location city : ${property.location?.city}")
                        _uiState.value = CreatePropertyUIState.Success(propertyId = editModeIDLoaded)
                    }else{
                        val propertyId= insertPropertyUseCase(property)
                ...
                if(property != null){
                    val location : LocationFormState? = if (property.location !=null){
                        ...
                        LocationFormState(
                            ...
                            streetNumber = FieldState(formFormater.formatInt(propertyLocation.streetNumber), true),
                    ...
                    _uiState.value = currentState.copy(
                        ...
                        dateEntry = InstantFieldState(property.dateEntry, true),
                        dateSale = InstantFieldState(property.dateSale, true),
